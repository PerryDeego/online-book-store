...
regstd_users.delete("/auth/delete-book/:isbn", (req, res) => {
  // Retrieve the ISBN from the request parameters and trim whitespace
  const isbn = req.params.isbn.trim();

  // Find book in the collection using the ISBN as a key
  const bookKey = Object.keys(books).find(key => books[key].isbn === isbn);

  // Validate if the book does not exist, return a 404 status
  if (!bookKey) {
...
regstd_users.put("/auth/update-review/:isbn", (req, res) => {
  ...
  // Provide a unique ID for the review
  const reviewId = Date.now().toString(); // Store the current timestamp as a unique ID (convert to string)
...
regstd_users.delete("/auth/delete-review/:isbn", (req, res) => {
  // Retrieve the isbn from the request parameters
  const isbn = req.params.isbn.trim(); // Trim any whitespace from the ISBN
  ...
  // Clear all reviews by setting it to an empty array
  book.reviews = []; // Remove all reviews
...
regstd_users.delete("/auth/review/:isbn/:reviewId", (req, res) => {
  ...
  const reviewIndex = book.reviews.findIndex(review => review.id === reviewId); // Assuming each review has a unique 'id'
